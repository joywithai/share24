 ...
 *
 * Two providers, one contract (`StorageProvider`):
 *
...
/** `STORAGE_TYPE`, defaulting to `LOCAL`. Throws on a value that is neither. */
export function configuredStorageType(
  ...
  throw new StorageConfigError(
    `STORAGE_TYPE must be "LOCAL" or "R2" (got "${env.STORAGE_TYPE}")`,
    { code: 'bad_storage_type' },
  );
}

/**
 * The provider for new uploads, or for an explicit storage type. The R2
 * provider is built (and its env validated) on first use, so a LOCAL
 * deployment never needs R2 credentials.
 */
...
/** Drops the memoised R2 provider — for tests, and after an env change. */
